...
You can see the JSON Schema for `components.json` [here](https://madui.dev/schema.json).

```json title="components.json"
{
  "$schema": "https://madui.dev/schema.json"
}
```
...
npx madui@latest add @v0/dashboard

# Install from private registry
npx madui@latest add @private/button

# Install multiple resources
npx madui@latest add @acme/header @internal/auth-utils
```
...
{
  "registries": {
    "@madui": "https://madui.dev/r/{name}.json",
    "@company-ui": {
...
- Install public components from MadUI
